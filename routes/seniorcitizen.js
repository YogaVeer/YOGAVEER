...
// Seed default senior citizen courses
router.get("/", async (req, res) => {
  ...
  for (const courseData of defaultCourses) {
    const exists = await SeniorCitizen.findOne({ name: courseData.name });
    if (!exists) {
      await SeniorCitizen.create(courseData);
    }
  }

  const courses = await SeniorCitizen.find();
  res.render("seniorcitizen/index", { courses });
});

// Show a specific senior citizen course with its sections and lessons
...
router.get("/lessons/new", async (req, res) => {
  const courses = await SeniorCitizen.find();
  const sections = await Section.find({ courseModel: "SeniorCitizen" }).populate("course");
  res.render("seniorcitizen/new", { courses, sections });
});
...
router.get("/lessons/:id/edit", async (req, res) => {
  ...
  const course = await SeniorCitizen.findById(section.course);
  const sections = await Section.find({ course: course._id });

  res.render("seniorcitizen/edit", { lesson, sections, course });
});

// Create a new lesson using video link
router.post("/lessons", async (req, res) => {
  const { courseId, sectionOption, sectionTitle, existingSectionId, lessonTitle, videoPath } = req.body;
  let section;

  if (sectionOption === "create") {
    section = new Section({ title: sectionTitle, course: courseId, courseModel: "SeniorCitizen" });
    await section.save();
  } else {
  ...
  await lesson.save();
  res.redirect(`/seniorcitizen/${courseId}`);
});

// Update an existing lesson (update video link)
router.put("/lessons/:id", async (req, res) => {
  ...
  const section = await Section.findById(lesson.section);
  res.redirect(`/seniorcitizen/${section.course}`);
});
...
router.delete("/lessons/:id", async (req, res) => {
  ...
  res.redirect(courseId ? `/seniorcitizen/${courseId}` : "/seniorcitizen");
});
